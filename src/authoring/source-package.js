...
// 组装生成物（deck.html、presentation.html）被预览窗口或 NAS 锁占用、重试耗尽时的错误码。
// 修法是释放占用或换 outDir；改入口文件名绕过会留下陈旧副本，形成两份页面真相。
const AUTHOR_ENTRY_WRITE_BUSY = 'AUTHOR_ENTRY_WRITE_BUSY';
const AUTHOR_ENTRY_WRITE_BUSY_HINT = '文件被占用（常见于预览窗口或 NAS 锁），关闭占用后重试或换 outDir；不要改入口文件名绕过';
...
function authorEntryWriteOptions(options = {}) {
  return {
    ...options,
    busyCode: AUTHOR_ENTRY_WRITE_BUSY,
    busyHint: AUTHOR_ENTRY_WRITE_BUSY_HINT,
  };
}
...
function auditAuthorPackageSourceFormat(configPath, options = {}) {
  ...
  for (const page of sourcePackage.pageFiles) {
    ...
    const runtimeAudit = auditStaticAuthoringRuntime(html, {
      file: page.relativePath,
      strict: false,
    });
    warnings.push(...runtimeAudit.warnings);
  }
