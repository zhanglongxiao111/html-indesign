...
`audit:conversion-gate` 用于汇总 `audit:effective-diff`、`audit:reverse-visual` 和可选 `audit:author-editability` 报告，输出单一 pass/fail 和失败原因。算法升级必须先保证 P0/P1、HTML 缺失、HTML 文本差异不回退；HTML 几何差异和可编辑性指标可以按明确预算逐步收紧，严禁为了过门禁扩大预算或隐藏差异。
