...
- 公司对外发布的 InDesign/HTML 统一 Skill 唯一发布源位于 `D:\AI\mcp-indesign\skills\indesign-cli\`。
- 本仓库不再维护 `.codex/skills/html-indesign-authoring/` 或其他可发布 Skill 副本。
- 本仓库继续维护 HTML 协议、作者规范、转换代码和测试；当这些变化影响 Agent 工作方法时，必须到 `mcp-indesign` 仓库同步更新统一 Skill 的 `references/html-authoring.md`。
- 面向 Agent 分发的模板目录和模板说明也归统一 Skill 源维护；本仓库中的 fixture、preset 和测试模板仍只服务代码与验证，不作为 Skill 发布副本。
- 公司内网发布命令、NAS 当前池和成功标准以 `D:\AI\mcp-indesign\AGENTS.md` 的“统一 Skill 内网发布”为唯一项目上下文；本仓库只保留此入口，不复制发布流程。
...
| `docs/superpowers/specs/2026-07-06-semantic-reconstruction-algorithm-roadmap.md` | 过程文档 | 语义重建阶段 0 准入、R1–R9 依赖、前置条件和验收切换路线图 |
| `docs/superpowers/plans/` | 过程目录 | 实施计划、阶段拆分和验证清单 |
| `docs/superpowers/plans/2026-07-11-semantic-reconstruction-stage0-implementation-plan.md` | 过程文档 | 语义重建阶段 0A–0F 的轻量实施计划 |
...
| 合成样式审计 | `npm run audit:synthesized-styles -- -- --model <semantic-model.json> [--out <report.json>]` |
...
| 真实人类 InDesign E2E + 二次回环 | `npm run e2e:indesign -- -- --indd <document.indd> --run-dir <local-test-workspace>` |
...
反向作者包审计会在 `reverse-html/author/reports/` 写出 `source-roundtrip-report.json`、`content-inventory-report.json` 和 `structure-signature-report.json`。三份报告都是首轮硬门槛：内容库存覆盖页面文本、资源、母版家具、几何对象、角色计数和计数守恒；结构签名覆盖节点存在、层级、顺序、class、文本与资源；源码回环的丢失类差异（页面缺失、文本变化、资源变化、tag 序列、字符样式 span、关键内联样式、回读记账泄漏）一律为 error，只有 config 元数据规范化保留为 warning（上述口径针对 structured 回读；observation / inferred 回读不保留源码结构，tag 序列、内联样式、资源写法、节点标签/顺序/父级这类写法差异降为 warning，内容类差异仍为 error，见 `docs/规范/REVERSE_EXPORT.md` 第 9 节）。首轮源码 byte 级漂移（sourceDrift）仍是辅助度量，不单独拦截。加 `--second-pass-roundtrip` 时，会把第一次回读出的 `reverse-html/author/deck.html` 再次送入 HTML-to-InDesign，并再次反向回读，用于验证 `HTML -> InDesign -> HTML -> InDesign -> HTML` 的作者包稳定性；二次回环硬门槛是 `canonical-content-inventory-report.json`、`canonical-structure-signature-report.json` 与 `canonical-source-drift-report.json` 均通过，其中作者包源码漂移必须为 0。

用 `--indd <document.indd>` 可以直接从真实人类 InDesign 文件开始一键回环：脚本会打开指定 `.indd` 导出 `reverse-snapshot.json`，生成 `reverse-html/author/`，再自动执行作者包二次回环，并把作者包源码零漂移作为硬失败条件。该模式默认使用 `observation` 回读，默认输出到 `test/workspace/human-indd-e2e-<时间戳>/`；涉及客户或 NAS 文件时必须显式使用本地 `--run-dir`，不要把回环输出写回源文件所在目录。
...
真实 E2E 默认使用 `presentation` 模式：浏览器视觉 CSS px 映射为 InDesign point，页面尺寸默认保持浏览器捕获尺寸。通过 npm 转发参数时需要双 `--`，例如 `npm run e2e:indesign -- -- --target-size qhd` 输出 2560x1440，也可以传 `--target-size 2048x1152`；目标比例必须和 HTML 页面比例一致。用 `npm run e2e:indesign -- -- --html <deck.html>` 可以指定其他固定语义 HTML。用 `npm run e2e:indesign -- -- --indd <document.indd> --run-dir <local-test-workspace>` 可以指定真实人类 InDesign 文件作为回环起点。建筑汇报 fixture 会默认把稳定样式 token 映射成人类可读中文样式名。
