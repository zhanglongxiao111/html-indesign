...
- [ ] 阶段 4：字段扫描和仅警告门禁（implementation：`f8b2e9b`；spec fix：`5c33f35`；quality fix：`4347e0a`；spec review 通过；code quality re-review 拒绝：data-id lexer 扫描 raw-text 内容；进行中：implementation fix）
- [ ] 阶段 5：接入语义模型和标签白名单
- [ ] 阶段 6：退役字段集中登记与清理
- [ ] 阶段 7：按域接入强校验
- [ ] 阶段 8：目录重构为适配器 / 写出器
