...
- [ ] 阶段 5：接入语义模型和标签白名单（implementation：`a8b4c0d`；spec fixes：`44ffe16`、`ad1ed4d`、`0687952`、`27deff8`、`d0994b1`、`e5fc097`；quality fixes：`6ca7c7e`、`e77dcf3`、`16147a1`、`9ca607a`；controller 复验通过：npm test 467/467；quality re-review 拒绝：`validateSemanticModel` 未对 raw `labels[]` 执行 kind-aware `validateLabelFields`，item label 的 document/page 字段可被 scanner 映射成合法路径；修复中）
- [ ] 阶段 6：退役字段集中登记与清理
- [ ] 阶段 7：按域接入强校验
- [ ] 阶段 8：目录重构为适配器 / 写出器
- [ ] 阶段 9：拆分大文件并保持行为可验证
