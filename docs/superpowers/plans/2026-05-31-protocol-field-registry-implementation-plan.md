...
- [ ] 阶段 3：能力矩阵和生命周期策略门禁（implementation：`8648f27`；spec fixes：`50386a6`、`b22aa2c`；quality fix：`b2fe754`；spec review 通过；code quality re-review 拒绝：registry retired lookup 仍暴露畸形 policy；进行中：implementation fix）
- [ ] 阶段 4：字段扫描和仅警告门禁
- [ ] 阶段 5：接入语义模型和标签白名单
- [ ] 阶段 6：退役字段集中登记与清理
- [ ] 阶段 7：按域接入强校验
