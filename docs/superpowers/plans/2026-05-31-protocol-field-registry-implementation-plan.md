...
- [ ] 阶段 6：退役字段集中登记与清理（implementation `af0065f`；implementation agent `019e81b1-af6e-7472-831a-dbfac7ae53d0` 完成；controller 复验通过：retired 4/4、retired+asset+author 39/39、protocol/semantic 51/51、`git diff --check` 通过；spec review agent `019e81b6-ff27-7380-a7f7-5b64ea04a8cd` PASS；quality review agent `019e81bb-599b-7ed1-a695-24d605de7265` BLOCKED：direction 空值静默当 read、retired policy 缺枚举校验、no-fallback grep 过脆弱；quality fix agent `019e81c0-5124-7fc2-b64f-2e599c1051c3` 进行中）
- [ ] 阶段 7：按域接入强校验
- [ ] 阶段 8：目录重构为适配器 / 写出器
- [ ] 阶段 9：拆分大文件并保持行为可验证
