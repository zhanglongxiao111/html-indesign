...
- [ ] 阶段 5：接入语义模型和标签白名单（implementation：`a8b4c0d`；spec fixes：`44ffe16`、`ad1ed4d`、`0687952`、`27deff8`；controller 复验：nested ghost strict 探针通过、stage focused 34/34、stage broad 116/116、npm test 452/452；spec re-review 拒绝：合法 reverse fixture strict 仍报 `labels[].name/token/displayName/styleKind/htmlClass` 和 `items[].effectiveLabel.styleRefs` 未登记；修复中）
- [ ] 阶段 6：退役字段集中登记与清理
- [ ] 阶段 7：按域接入强校验
- [ ] 阶段 8：目录重构为适配器 / 写出器
- [ ] 阶段 9：拆分大文件并保持行为可验证
