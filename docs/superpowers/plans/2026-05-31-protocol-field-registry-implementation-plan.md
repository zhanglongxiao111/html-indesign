...
- [ ] 阶段 5：接入语义模型和标签白名单（implementation：`a8b4c0d`；spec fixes：`44ffe16`、`ad1ed4d`、`0687952`、`27deff8`、`d0994b1`、`e5fc097`；controller 复验通过：tagged reverse strict、nested ghost strict、retired attr docs、focused 49/49、stage broad 120/120、npm test 456/456；spec review 通过；code quality review 拒绝：label strict 缺少 kind 约束、table cell nested strict 存盲点、reverse preset 缺失静默回默认；修复中）
- [ ] 阶段 6：退役字段集中登记与清理
- [ ] 阶段 7：按域接入强校验
- [ ] 阶段 8：目录重构为适配器 / 写出器
- [ ] 阶段 9：拆分大文件并保持行为可验证
