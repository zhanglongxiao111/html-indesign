...
- [ ] 阶段 5：接入语义模型和标签白名单（implementation：`a8b4c0d`；spec fixes：`44ffe16`、`ad1ed4d`、`0687952`、`27deff8`、`d0994b1`、`e5fc097`；controller 复验中：重点复核 retired attr 文档仅使用 `retired.htmlAttrs.dataIdPage` / `getRetiredHtmlAttr()`，无旧 `html.retiredAttrs` / `html.data-id-page` 示例）
- [ ] 阶段 6：退役字段集中登记与清理
- [ ] 阶段 7：按域接入强校验
- [ ] 阶段 8：目录重构为适配器 / 写出器
- [ ] 阶段 9：拆分大文件并保持行为可验证
