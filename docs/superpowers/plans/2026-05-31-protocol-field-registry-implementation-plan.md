...
- [ ] 阶段 5：接入语义模型和标签白名单（implementation：`a8b4c0d`；controller 验证：focused/protocol/reverse 104/104、npm test 440/440；spec review 拒绝：reverse diagnostics 未 surfaced、`parentPageId`/`parentPageName` 未登记却进入模型；修复中）
- [ ] 阶段 6：退役字段集中登记与清理
- [ ] 阶段 7：按域接入强校验
- [ ] 阶段 8：目录重构为适配器 / 写出器
- [ ] 阶段 9：拆分大文件并保持行为可验证
