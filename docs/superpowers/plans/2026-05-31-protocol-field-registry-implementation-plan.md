...
- [ ] 阶段 5：接入语义模型和标签白名单（implementation：`a8b4c0d`；spec fixes：`44ffe16`、`ad1ed4d`、`0687952`、`27deff8`、`d0994b1`；controller 复验通过：tagged reverse fixture strict 通过、effectiveLabel.styleRefs ghost strict 失败、label payload fields 为 sourceMetadata、`data-id-page` 未复活、focused 49/49、stage broad 120/120、npm test 456/456；spec re-review 拒绝：spec/plan 文档仍残留 `html.retiredAttrs` / `html.data-id-page` 旧退役字段示例；修复中）
- [ ] 阶段 6：退役字段集中登记与清理
- [ ] 阶段 7：按域接入强校验
- [ ] 阶段 8：目录重构为适配器 / 写出器
- [ ] 阶段 9：拆分大文件并保持行为可验证
