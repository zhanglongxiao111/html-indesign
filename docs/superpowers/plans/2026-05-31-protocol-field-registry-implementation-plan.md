...
- [ ] 阶段 5：接入语义模型和标签白名单（implementation：`a8b4c0d`；spec fix：`44ffe16`；controller 复验：focused/protocol/reverse 109/109、npm test 445/445；spec re-review 拒绝：合法 `DocumentModel` 字段和 reverse 诊断字段未完整登记；修复中）
- [ ] 阶段 6：退役字段集中登记与清理
- [ ] 阶段 7：按域接入强校验
- [ ] 阶段 8：目录重构为适配器 / 写出器
- [ ] 阶段 9：拆分大文件并保持行为可验证
