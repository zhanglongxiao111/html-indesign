...
进度：二次审核退回修复中（Task 3 review 要求继续修复 G3.1 嵌套死 helper 假阳性、G8 test-only 引用误算 ownership、G4.1 注释/字符串冒充 invalid-input 测试；控制层纠偏：`generate-field-docs.js` 是合法生成器入口，不得回填 G8 baseline，2026-07-07）。
