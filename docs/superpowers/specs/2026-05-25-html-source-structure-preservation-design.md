...
- 对无标签或标签不完整的手工 InDesign，输出观察 HTML 和补标报告，不假装恢复源码结构。
...
| `observation` | 观察无标签 InDesign | 无标签也可 | 绝对坐标视觉 HTML、未知对象报告、待补标签清单 |

`authoring` 是本设计新增的高质量目标；`structured` 是当前已经跑通的主线；`observation` 是迁移和补标入口。

`authoring` 不应静默降级到 `structured`。如果缺少必要源结构字段，应失败并输出报告，提示使用 `structured` 或 `observation`。
...
`observation` writer 输出视觉 HTML，并明确标记 `data-id-reverse-mode="observation"`。
...
| `AUTHORING_DOWNGRADE_REQUIRED` | error | 只能 structured/observation，不能 authoring |
...
- `REVERSE_EXPORT.md` 应吸收 `authoring` 模式和降级规则。
...
- `structured` 和 `observation` 模式仍可独立工作，不被 authoring 模式复杂度污染。

## 14. 关键决策

本设计采用：

- **协议标签优先**：完整还原依赖正向写入的源结构标签。
- **InDesign 组辅助**：组用于人类编辑体验和显式组件关系，但不是唯一嵌套来源。
- **参考线校验网格**：参考线帮助人类和校验器，但页面标签才是网格语义来源。
- **模式严格分离**：`authoring`、`structured`、`observation` 不互相假装。
- **无标签不猜源码**：无标签 InDesign 只能观察和补标，不能声称恢复 authoring HTML。
