...
## 2. 人类命名与 InDesign 图层规则

InDesign 文档图层和人类命名不作为本阶段角色语义证据。
...
- 段落、字符、对象样式和复合字体也可能只因“格式接近”被临时挪用；名称看起来像“标题”不代表对象就是标题。
...
- 段落、字符、对象样式和复合字体的显示名只能作为观察信息；算法不得解析名称文字并据此提高角色置信度。
- 已登记的样式 token 可以作为“同格式分组”旁证，但有效格式应以合成样式事实为主；token 相同不能直接升级成标题、正文、图注等角色。
- z-order 仍然可用，因为它描述页面对象实际前后遮挡和绘制顺序，不等同于人类维护的文档图层。
- PDF、AI、PSD 等置入资源自身的图层显隐属于资源置入参数，不属于 InDesign 文档图层语义，可以继续按资源事实保留。

必须有测试证明：对象位于名为 `标题`、`正文`、`图纸` 等图层，或引用同名段落/字符/对象样式时，页面对象图不能因此生成对应语义候选或提高角色分数。
...
- 对象 bounds。
- 对象 z-order 或页面绘制顺序。
- 文本对象的纯文本、文字样式摘要（至少包括 `pointSize`、`leading`、字重、对齐）和溢出状态。
...
当前实施状态（2026-07-11）：反向模型已提供 `textStyle.pointSize` 与 `textStyle.leading`，对象图实现仍只尝试读取 `fontSize`，尚未把 `leading` 写入 `textFacts`。在完成路线图 A0 并增加回归测试前，R1/R4 不得宣称已消费真实字号或行高证据。
...
| `rotation` | 旋转角度 |
| `zOrder` | 页面对象前后顺序 |
| `textFacts` | 文本长度、行数、首行、`fontSize`（映射自 canonical `pointSize`）、`leading`、字重摘要、对齐方式、溢出状态 |
...
- 对象引用名为 `标题` 的段落/字符/对象样式时，同样不生成标题语义候选、不提高角色分数；样式 token 只允许作同格式分组旁证。
- 输入只提供 `textStyle.pointSize` / `textStyle.leading` 时，`textFacts.fontSize` / `textFacts.leading` 必须得到对应数值，不能因字段名不一致变成 `null`。
