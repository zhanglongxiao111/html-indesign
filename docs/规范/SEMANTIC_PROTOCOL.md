...
字段名、token 作用域和模型路径由协议字段注册表统一管理。当前字段事实源是 `PROTOCOL_FIELD_REGISTRY.md` 及其生成来源 `src/protocol/` registry；本文只保留固定语义规则、authoring 约束、历史迁移说明和关键示例。
...
当前的 blueprint 提取、参考 HTML 生成和 `AGENT_SPEC.md` 属于历史模板迁移层，主要用于追溯和迁移旧 InDesign 模板。旧 blueprint 不作为新 authoring 协议的事实源，但可以作为 `indesign-reverse` 的 historical blueprint input，被转换为统一语义模型后输出 observation/inferred HTML。
...
本节到第 5 节记录旧模板迁移协议，用于理解和迁移 `data-master` / `data-slot` / `blueprint` / historical build instructions。它们不是当前 authoring 协议的事实源。当前主线事实源是 `HTML_INDESIGN_LIBRARY_SPEC.md` 中定义的统一语义模型，InDesign 端持久化边界是 `LABEL_PROTOCOL.md` 中定义的 `html_indesign` 标签。旧 blueprint 输入必须先归一化进统一语义模型，再写出反向 HTML。
...
Blueprint 是历史模板迁移层的事实来源，只服务旧模板查看、旧槽位迁移和历史测试资产。新 authoring 主线不得把 blueprint 当作长期事实源，也不得要求新 HTML 先退化成 `master/slot` 结构。反向迁移时，blueprint 可通过 `blueprintMigrationToSemanticModel` 进入 `indesign-reverse`，输出带置信度的 inferred/observation HTML。

当前 historical blueprint 有效结构包含：
...
## 5. 历史 Build Instructions

本节描述旧模板迁移层的 build instructions。新的 `paged-html` 主线仍会生成 InDesign executor 消费的 JSON 指令，但该指令是统一语义模型派生出的执行命令，不是长期事实源；当前主线指令结构以 `HTML_INDESIGN_LIBRARY_SPEC.md` 的 “Build Instructions” 章节为准。

旧模板 HTML 编译后生成单个 JSON 指令文件，默认路径为 `test/workspace/instructions.json`，供历史 builder / executor 消费。
...
- `data-id-pdf-page` 专用于 PDF 置入页码；历史 `data-id-page` 不再作为 PDF 页码读取，只能作为无效观察字段或迁移问题记录。
...
反向导出必须优先读取 `html_indesign`，只在标签缺失时进入观察模式或历史 blueprint 迁移路径。
